...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    if (indexPath.row >= self.startIndex - 1 && indexPath.row <= self.dayCount + self.startIndex - 2) {
        NSString *dateString =  [[NSString alloc] initWithFormat:@"%ld", indexPath.row - self.startIndex + 2];
        cell.label.text = [dateString stringToNepaliNumber];
    } else {
        cell.label.text = @" ";
    }
    return cell;
